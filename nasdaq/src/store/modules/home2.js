...
const actions = {
  ...
  // 主页nav对应模块数据
  async reqIndexCateModule({ commit }) {
    // 调用接口发送异步请求
...
const getters = {
  ...
  reqIndexcategoryHotSellModule1(state) {
    const reqIndexcategoryHotSellModule1 = state.reqIndex2.categoryHotSellModule.categoryList.splice(
      0,
      2
    );
  ...
  reqIndexcategoryHotSellModule2(state) {
    const reqIndexcategoryHotSellModule2 = state.reqIndex3.categoryHotSellModule.categoryList.splice(
      2,
      10
    );
